...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val scroll = ScrollView(this).apply { setBackgroundColor(Color.rgb(7,8,14)) }
        ...
        fun text(value: String, size: Float, color: Int = Color.WHITE, top: Int = 0): TextView {
            return TextView(this).apply {
                text = value; textSize = size; setTextColor(color); setPadding(0, dp(top), 0, dp(6))
            }
        }
        fun section(title: String) {
            root.addView(text(title, 13f, Color.rgb(170,145,255), 18))
        }
        ...
        fun toggle(title: String, checked: Boolean, enabled: Boolean = true, onChange: (Boolean)->Unit): Switch {
            return Switch(this).apply {
                text = title; textSize = 15f; setTextColor(Color.WHITE); isChecked = checked; isEnabled = enabled
                setPadding(0, dp(4), 0, dp(4))
                setOnCheckedChangeListener { _, value -> onChange(value); preview.reload() }
                root.addView(this)
            }
        }
        ...
        val rangeLabel = text("Speed  %.2f – %.2f".format(FlowSettings.loadSpeedMin(this), FlowSettings.loadSpeedMax(this)), 15f)
        root.addView(rangeLabel)
        root.addView(RangeSliderView(this).apply {
            configure(0.05f, 2.5f, FlowSettings.loadSpeedMin(this@MainActivity), FlowSettings.loadSpeedMax(this@MainActivity))
        ...
        graphics.addView(blurRadio); graphics.addView(flutedRadio); root.addView(graphics)
